-- 016 - Persist multiclass class levels

alter table public.characters
  add column if not exists class_levels jsonb not null default '[]'::jsonb;

-- Existing single-class characters intentionally keep an empty class_levels array.
-- The application treats the legacy class_id/subclass_id/level fields as their
-- canonical single-class representation until a multiclass breakdown is saved.

create index if not exists characters_class_levels_gin_idx
  on public.characters using gin (class_levels);
...
begin
  ...
  set
    ...
    class_levels = coalesce(v_character.class_levels, '[]'::jsonb),
  ...
  select p_character_id, row_data.spell_id, row_data.prepared, row_data.dm_granted, row_data.source
  from jsonb_to_recordset(p_spells) as row_data(
    spell_id uuid, prepared boolean, dm_granted boolean, source text
  );
  ...
  select p_character_id, row_data.feature_id, row_data.dm_granted, row_data.source
  from jsonb_to_recordset(p_features) as row_data(
    feature_id uuid, dm_granted boolean, source text
  );
  ...
  select p_character_id, row_data.item_id, row_data.quantity, row_data.equipped, row_data.dm_granted
  from jsonb_to_recordset(p_items) as row_data(
    item_id uuid, quantity integer, equipped boolean, dm_granted boolean
  );
  ...
  select p_character_id, row_data.optional_feature_id, row_data.dm_granted, row_data.source
  from jsonb_to_recordset(p_optional_features) as row_data(
    optional_feature_id uuid, dm_granted boolean, source text
  );
  ...
  select p_character_id, row_data.homebrew_content_id, row_data.dm_granted
  from jsonb_to_recordset(p_homebrew) as row_data(
    homebrew_content_id uuid, dm_granted boolean
  );
  ...
  select p_character_id, row_data.kind, row_data.level, row_data.data
  from jsonb_to_recordset(p_progression_history) as row_data(
    kind text, level integer, data jsonb
  );
end;
$$;
