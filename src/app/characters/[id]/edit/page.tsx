...
export default function EditCharacterPage() {
  ...
  const { characters, updateCharacter, catalogue, raceRules, backgroundRules, featureCatalogue, featCatalogue } = useCharacters();
  ...
  return <CharacterEditor character={character} catalogue={catalogue} raceRules={raceRules} backgroundRules={backgroundRules} featureCatalogue={featureCatalogue} featCatalogue={featCatalogue} onSave={async (patch) => { await updateCharacter(character.id, patch); router.push(`/characters/${character.id}`); }} />;
}

function CharacterEditor({ character, catalogue, raceRules, backgroundRules, featureCatalogue, featCatalogue, onSave }: { character: Character; catalogue: ReturnType<typeof useCharacters>["catalogue"]; raceRules: ReturnType<typeof useCharacters>["raceRules"]; backgroundRules: ReturnType<typeof useCharacters>["backgroundRules"]; featureCatalogue: ReturnType<typeof useCharacters>["featureCatalogue"]; featCatalogue: ReturnType<typeof useCharacters>["featCatalogue"]; onSave: (patch: Partial<Character>) => void | Promise<void> }) {
  ...
  function setRace(value: string) {
    ...
    setAbilities(applyAbilityBonuses(baseAbilities, raceRules[value]?.abilityBonuses ?? {}));
  }
  function setBackground(value: string) {
    const data = backgroundRules[value];
    setForm((current) => ({ ...current, background: value }));
    if (data?.skills.length) {
      setSkills((current) => [...new Set([...current, ...data.skills])].join(", "));
    }
    if (data?.languages.length) {
      setLanguages((current) => [...new Set([...current, ...data.languages])].join(", "));
    }
  }
