...
export default function NewCharacterPage() {
  ...
  const { createCharacter, catalogue, raceRules, backgroundRules, featCatalogue } = useCharacters();
  ...
  useEffect(() => {
    ...
    setForm((current) => {
      ...
      const abilities = applyAbilityBonuses(baseAbilities, raceRules[race]?.abilityBonuses ?? {});
      const backgroundData = backgroundRules[background];
      setSkills(backgroundData?.skills ?? []);
      setLanguages(backgroundData?.languages ?? []);
      return { ...current, className, race, background, subclass, abilities };
    });
  }, [catalogue, raceRules, backgroundRules, baseAbilities]);
  ...
  function setRace(value: string) {
    setForm((current) => ({ ...current, race: value, abilities: applyAbilityBonuses(baseAbilities, raceRules[value]?.abilityBonuses ?? {}) }));
  }
  ...
  async function submit() {
    ...
    const character = {
      ...form,
      abilities: applyAbilityBonuses(baseAbilities, raceRules[form.race]?.abilityBonuses ?? {}),
      skills,
      languages,
      maxHp,
